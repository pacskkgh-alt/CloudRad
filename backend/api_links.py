...
def verify_link(request: Request, token: str, req: VerifyLinkRequest, db: Session = Depends(database.get_db)):
    ...
    return {
        ...
        "patient_name": "مريض مجهول الهوية" if link.is_anonymized else (study.patient.full_name if study.patient else "غير متوفر"),
        "patient_id_number": "مخفي" if link.is_anonymized else (study.patient.patient_id_number if study.patient else "غير متوفر"),
